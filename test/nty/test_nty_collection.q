...
sub test() {
    ...
    my *string $user = shift $ARGV;
    my *string $pass = shift $ARGV;
    my *string $dbname = shift $ARGV;

    my Datasource $db("oracle", exists $user ? $user : "omquser", exists $pass ? $pass : "omquser", exists $dbname ? $dbname : "qube");
    $db.open();
    ...
    my list $colo = (bindOracleObject($user + ".TEST_OBJECT", $obj),
		     bindOracleObject($user + ".TEST_OBJECT", $obj),
		     bindOracleObject($user + ".TEST_OBJECT", $obj));
    ...
    $d = $db.exec("begin qore_test.do_coll_timestamp_tz(%v, :retval); end;",
...
sub test_coll_out_obj(Datasource $db) {
