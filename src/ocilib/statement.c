                     ...
                     OCI_TypeInfo *typinf, unsigned int nbelem)
{
    ...
    printf("OCILIB: OCI_Bind() buf=0x%x size=%lld\n", bnd->buf.data, bnd->size);
    ...
    if ((res == TRUE) && (mode == OCI_BIND_INPUT))
    {
        if (stmt->bind_alloc_mode == OCI_BAM_EXTERNAL)
        {
		    ...
		    printf("OCILIB: OCI_Bind() A buf=0x%x size=%lld nbelem=%ld\n", bnd->buf.data, size, nbelem);
                }

                res = (bnd->buf.data != NULL);
            }
            else {
	       printf("OCILIB: OCI_Bind() B buf=0x%x size=%lld\n", bnd->buf.data, bnd->size);
   ...
   /* OCI binding */
    ...
    if (res == TRUE)
    {
        if (stmt->bind_mode == OCI_BIND_BY_POS)
        {
	       ...
	       printf("OCILIB: OCI_BindData() OCIBindByPos data=0x%x size=%ld inds=0x%x\n", bnd->buf.data, bnd->size, bnd->buf.inds);
        }
        else
        {
            void * ostr = NULL;
	       ...
	       printf("OCILIB: OCI_BindData() OCIBindByName data=0x%x size=%ld inds=0x%x\n", bnd->buf.data, bnd->size, bnd->buf.inds);
        ...
        if (code == SQLT_NTY || code == SQLT_REF)
        {
	       ...
	       printf("OCILIB: OCI_BindData() OCIBindObject data=0x%x size=%ld tdo=0x%x obj_inds=0x%x inds=0x%x\n", bnd->buf.data, bnd->size, typinf->tdo, bnd->buf.obj_inds, bnd->buf.inds);
